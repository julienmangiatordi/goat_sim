# GOAT Robot Simulation Guide for Claude AI

**Reference:** Polzin et al., *Sci. Robot.* 10, eadp6419 (2025)  
**Status:** Genesis Physics Engine | Python Control | URDF Mechanics  
...
## Table of Contents
1. [Overview](#overview)
2. [URDF Architecture](#urdf-architecture)
3. [Python Code Structure](#python-code-structure)
4. [Physics Model](#physics-model)
5. [Simulation Scenarios](#simulation-scenarios)
6. [Parameters & Tuning](#parameters--tuning)
...
### GOAT Robot Concept
GOAT is a **reconfigurable morphing robot** that changes shape via cable-driven tendon actuation:
- **Circle mode**: Compact (r=1.0), good turning
- **Rover mode**: Elongated (r=2.2), optimal for straight-line speed
- **Sphere mode**: Folded (r<1.0), compact storage

### Simulation Architecture
```
URDF (goat_rover_2.urdf)
    ↓
Genesis Physics Engine [C++/GPU]
    ↓
Python Controller (goat_rover_sim.py)
    ↓
Viewer & Data Logger
```

**Key Physics Components in Python:**
- Tendon cable forces (simulated via `apply_links_external_force`)
- Wheel motor control (velocity-based skid-steer)
- Pivot compliance modeling (fiberglass frame flexing)
- Contact deformation (gap scenario)
...
## Quick Reference: Modifying for Claude AI

When asking Claude AI to modify the simulation, provide:

1. **What to change:** File name, line number range, current code
2. **Why:** Physics reason or paper section
3. **Expected behavior:** What should happen after change
4. **Test scenario:** Which scenario to validate with

**Example:**
```
File: goat_rover_sim.py, line 50
Current: TENDON_K = 2000.0
Change to: TENDON_K = 3000.0
Reason: Gap scenario deformation too slow
Expected: Robot compresses faster when hitting walls
Test: python goat_rover_sim.py --scenario gap
```

---

**Document Version:** 1.0 | Last Generated: April 2026
